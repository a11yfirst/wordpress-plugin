...
/*
a11yFirst plugins are build on CKSouce. 
About A11yfirst Plugin, 
please visit https://a11yfirst.library.illinois.edu/
About CKSource, 
Copyright (c) 2003-2015, CKSource - Frederico Knabben. All rights reserved.
For licensing, see LICENSE.md or http://ckeditor.com/license
*/

class a11yfirsteditor_wordpress {
	...
	public function __construct() {
		...
		define('A11YFIRSTEDITOR_PLUGIN_URL', $this->plugin_path);
		$this->a11yfirsteditor_path = $this->plugin_path . 'a11yfirsteditor/';
		$this->editable_files = array(
			'a11yfirsteditor.config.js' => dirname(__FILE__) . '/a11yfirsteditor.config.js',
			'a11yfirsteditor.styles.js' => dirname(__FILE__) . '/a11yfirsteditor.styles.js',
			'a11yfirsteditor.templates.js' => dirname(__FILE__) . '/a11yfirsteditor.templates.js',
		);
		...
		$options = get_option('a11yfirsteditor_wordpress');
		if (!$options) {
			add_option('a11yfirsteditor_wordpress', $this->default_options);
	...
	private function checkbox($section, $var, $text, $onClick = '') {
		return '<label id="lbl_' . $var . '"><input type="checkbox" id="' . $var . '" name="options[' . $section . '][' . $var . ']"' .
			...
			($this->options[$section][$var] == 't' ? "checked" : '') . '>&nbsp;' . __($text, 'a11yfirsteditor_wordpress') . "</label>\n";
	}

	public function add_admin_head() {
		echo '<style type="text/css">
			.form-table td .cke_editor td { padding:0; }
			#icon-wp-ckeditor {background: transparent url("'.$this->plugin_path.'images/a11yfirsteditor_ico32.png") no-repeat;}
		</style>';
		...
		if (is_plugin_active('qtranslate/qtranslate.php'))
		{
			...
			wp_enqueue_script( 'a11yfirsteditor', $this->a11yfirsteditor_path .'ckeditor.js?t='.$this->timestamp, array(), $this->version);
			wp_enqueue_script( 'a11yfirsteditor.utils', $this->plugin_path . 'includes/a11yfirsteditor.utils.js?t='.$this->timestamp, array('a11yfirsteditor', 'jquery'), $this->version);
			global $q_config;
	...
	public function print_admin_styles() {
		wp_enqueue_style('a11yfirsteditor_admin', $this->plugin_path . 'includes/overview.css', false, $this->version, 'screen');
	}

	public function print_admin_upload_styles() {
		wp_enqueue_style('a11yfirsteditor_admin', $this->plugin_path . 'includes/upload.css', false, $this->version, 'screen');
	}

	public function add_option_page() {
		add_menu_page(__('a11yfirstEditor Settings'), 'a11yfirstEditor', 'administrator', 'a11yfirsteditor_settings', array(&$this, 'plugin_overview'), $this->plugin_path . 'images/menuicon.gif');
		//line below replace url title from A11yFirstEditor to Overview
		$overview_page = add_submenu_page('a11yfirsteditor_settings', __('a11yfirstEditor Overview'), __('Overview'), 'administrator', 'a11yfirsteditor_settings', array(&$this, 'plugin_overview'));
		...
		if (isset($_GET['page']) && $_GET['page'] == 'a11yfirsteditor_settings') {
			wp_enqueue_script('postbox');
		}

		$basic_page = add_submenu_page('a11yfirsteditor_settings', __('a11yfirstEditor Basic Settings'), __('Basic Settings'), 'administrator', 'a11yfirsteditor_basic_options', array(&$this, 'option_page'));
		add_action('admin_print_scripts-' . $basic_page, array(&$this, 'basic_settings_js'));

		add_submenu_page('a11yfirsteditor_settings', __('a11yfirstEditor Advanced Settings'), __('Advanced Settings'), 'administrator', 'a11yfirsteditor_advanced_options', array(&$this, 'advanced_options'));
		$upload_page = add_submenu_page('a11yfirsteditor_settings', __('a11yfirstEditor Upload Settings'), __('Upload Options'), 'administrator', 'a11yfirsteditor_upload_options', array(&$this, 'upload_options'));
		add_action('admin_print_styles-' . $upload_page, array(&$this, 'print_admin_upload_styles'));

		if (count($this->get_writable_files()) > 0) {
			$file_editor_page = add_submenu_page('a11yfirsteditor_settings', __('a11yfirstEditor File Editor'), __('File Editor'), 'administrator', 'a11yfirsteditor_file_editor', array(&$this, 'file_editor'));
			add_action('admin_print_scripts-' . $file_editor_page, array(&$this, 'file_editor_js'));
		}
	}

	private function a11yfirsteditor_get_version() {
		$jspath = $this->a11yfirsteditor_path . 'ckeditor.js';
		...
		return __('N/A', 'a11yfirsteditor_wordpress');
	}
	...
	private function ckfinder_status() {
		...
		if (!file_exists($ckfinder_php)) {
			if (file_exists($nested_ckfinder_php)) {
					return '<span class="ckeditor_error">' . __('CKFinder installed in wrong (nested) directory. Move files from a11yfirsteditor/ckfinder/ckfinder to a11yfirsteditor/ckfinder directory.', 'a11yfirsteditor_wordpress') . '</span>';
			} else {
					return __('Not installed', 'a11yfirsteditor_wordpress');
			}
		} else {
			$contents = @file_get_contents($config_path);
			if (!$contents) {
				return '<span class="ckeditor_error">' . __('CKFinder is installed, configuration file not readable or empty.', 'a11yfirsteditor_wordpress') . '</span>';
			}
			if (strpos($contents, 'wp-config.php') === false) {
				return '<span class="ckeditor_error">' . __('CKFinder is installed, but invalid config.php was detected. Rename ckfinder_config.php to config.php (overwriting the default config.php file distributed with CKFinder).', 'a11yfirsteditor_wordpress') . '</span>';
			}
			if ($this->options['upload']['browser'] != 'ckfinder') {
				return '<span class="ckeditor_ok">' . __('CKFinder is installed, not enabled.', 'a11yfirsteditor_wordpress') . '</span>';
			}
			return '<span class="ckeditor_ok">' . __('CKFinder is installed', 'a11yfirsteditor_wordpress') . '</span>';
		}
	}

	public function plugin_overview() {
		global $a11yfirsteditor_version, $a11yfirsteditor_plugin_version, $ckfinder_status;
		$a11yfirsteditor_version = $this->a11yfirsteditor_get_version();
		$a11yfirsteditor_plugin_version = $this->version;
		$ckfinder_status = $this->ckfinder_status();
		if (isset($_POST['reset']) && $_POST['reset'] == 1) {
			if (!wp_verify_nonce($_POST['csrf_a11yfirsteditor-for-wordpress'], 'a11yfirsteditor_create_nonce_overview') || empty($_POST['_wp_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_wp_http_referer']) )) {
				wp_die( "You do not have sufficient permissions to access this page." );
			}
			update_option('a11yfirsteditor_wordpress', $this->default_options);
	...
	public function option_page() {
		if (!empty($_POST['submit_update'])) {
			$message = array();
			/* validation */
			if (!wp_verify_nonce($_POST['csrf_a11yfirsteditor-for-wordpress'], 'a11yfirst_create_nonce_basic') || empty($_POST['_wp_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_wp_http_referer']) )) {
				wp_die( "You do not have sufficient permissions to access this page." );
			}
	...
	protected function get_skins(){
		$skins_directory = $this->a11yfirsteditor_path . 'skins/';
	...
	public function upload_options() {
		if (!empty($_POST['submit_update'])) {
			if (!wp_verify_nonce($_POST['csrf_a11yfirsteditor-for-wordpress'], 'a11yfirsteditor_create_nonce_upload') || empty($_POST['_wp_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_wp_http_referer']) )) {
				wp_die( "You do not have sufficient permissions to access this page." );
			}
			$new_options = $_POST['options'];
			foreach (array('access', 'fileView', 'fileDelete', 'fileRename', 'fileUpload', 'folderView', 'folderDelete', 'folderCreate', 'folderRename') as $command) {
				$this->set_capability($new_options['ckfinder']['permissions'][$command], "a11yfirsteditor_ckfinder_" . $command);
			}
			...
			if ($new_options['upload']['browser'] != 'disabled') {
				...
				foreach ($inputs as $input) {
					if (empty($new_options['upload'][$input])) {
						$message['upload_' . $input] = __('This field shouldn\'t be empty', 'a11yfirsteditor_wordpress');
					} elseif (!preg_match('#^([a-z0-9]+){1}(,[a-z0-9]+)*,?$#Uis', trim($new_options['upload'][$input]))) {
						$message['upload_' . $input] = __('Files extensions has wrong chars.', 'a11yfirsteditor_wordpress');
					}
				}

				if ($new_options['upload']['browser'] == 'ckfinder') {
					$checkCKFinder = $this->ckfinder_status();
					if (!strpos($checkCKFinder, "a11yfirsteditor_ok")) {
						$message['upload_browser'] = $checkCKFinder;
					}
					if (empty($new_options['ckfinder']['file_max_size'])) {
						$message['ckfinder_file_max_size'] = __('This field is required.', 'a11yfirsteditor_wordpress');
					} elseif (!preg_match('/^\d+[MKG]?$/i', trim($new_options['ckfinder']['file_max_size']))) {
							$message['ckfinder_file_max_size'] = __('Enter valid value. Example: 400 or 10M', 'a11yfirsteditor_wordpress');
					}
					$inputs = array('images_width', 'images_height', 'thumbnails_width', 'thumbnails_height');
					foreach ($inputs as $input) {
						if (empty($new_options['ckfinder'][$input])) {
							$message['ckfinder_' . $input] = __('This field is required.', 'a11yfirsteditor_wordpress');
						} elseif (!preg_match('#^\d+$#', trim($new_options['ckfinder'][$input]))) {
							$message['ckfinder_' . $input] = __('Enter valid value. Example: 400', 'a11yfirsteditor_wordpress');
						}
					}
					if (empty($new_options['ckfinder']['thumbnails_quality'])) {
						$message['ckfinder_thumbnails_quality'] = __('This field is required.', 'a11yfirsteditor_wordpress');
					} elseif (!(preg_match('#^[0-9]{1,2}$#', trim($new_options['ckfinder']['thumbnails_quality'])) || trim($new_options['ckfinder']['thumbnails_quality']) == '100')) {
						$message['ckfinder_thumbnails_quality'] = __('Value should be betwen 1 and 100.', 'a11yfirsteditor_wordpress');
					}
					if (empty($new_options['ckfinder']['images_quality'])) {
						$message['ckfinder_images_quality'] = __('This field is required.', 'a11yfirsteditor_wordpress');
					} elseif (!(preg_match('#^[0-9]{1,2}$#', trim($new_options['ckfinder']['images_quality'])) || trim($new_options['ckfinder']['images_quality']) == '100')) {
						$message['ckfinder_images_quality'] = __('Value should be betwen 1 and 100.', 'a11yfirsteditor_wordpress');
					}
				}
			}
	...
	public function advanced_options() {
		if (!empty($_POST['submit_update'])) {
			if (!wp_verify_nonce($_POST['csrf_a11yfirsteditor-for-wordpress'], 'a11yfirsteditor_create_nonce_advanced') || empty($_POST['_wp_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_wp_http_referer']) )) {
				wp_die( "You do not have sufficient permissions to access this page." );
			}
			$new_options = $_POST['options'];
			...
			if (!empty($new_options['css']['path'])) {
				if ($new_options['css']['mode'] != 'self') {
					$message['css_path'] = __('CSS path is not empty. Please set the "Editor CSS" option to "define css" mode.', 'a11yfirsteditor_wordpress');
				} elseif (FALSE !== strpos($new_options['css']['path'], '"')) {
					$massage['css_path'] = __('Double quotes are not allowed in CSS path.', 'a11yfirsteditor_wordpress');
				} elseif (substr($new_options['css']['path'], 0, 1) == "'" && substr($new_options['css']['path'], -1) == "'") {
					$massage['css_path'] = __('Enter valid path, do not surround it with quotes.', 'a11yfirsteditor_wordpress');
				}
			}
			/** styles * */
			if (!empty($new_options['css']['style_path'])) {
				if ($new_options['css']['styles'] != 'self') {
					$message['css_style_path'] = __('Path to predefined styles is not empty. Please set the "Predefined styles" option to "define path to a11yfirsteditor.styles.js" mode.', 'a11yfirsteditor_wordpress');
				} elseif (FALSE !== strpos($new_options['css']['styles_path'], '"')) {
					$message['css_style_path'] = __('Double quotes are not allowed in path.', 'a11yfirsteditor_wordpress');
				} elseif (substr($new_options['css']['style_path'], 0, 1) == "'" && substr($new_options['css']['style_path'], -1) == "'") {
					$message['css_style_path'] = __('Enter valid path, do not surround it with quotes.', 'a11yfirsteditor_wordpress');
				}
			}
			/** load timeout * */
			if (!preg_match('#^\d+$#', trim($new_options['advanced']['load_timeout']))) {
				$massage['advanced_load_timeout'] = __('Enter valid load timeout in seconds.', 'a11yfirsteditor_wordpress');
			}

			/* language settings */
			if (!preg_match('#^\d\d$#', trim($new_options['advanced']['language'])) && !preg_match('#^\d\d-\d\d$#', trim($new_options['advanced']['language']))) {
				$massage['advanced_language'] = __('Enter a valid language.', 'a11yfirsteditor_wordpress');
			}

			if (trim($new_options['advanced']['detect_language_auto']) != 't' && trim($new_options['advanced']['detect_language_auto'] != 'f')) {
				$massage['advanced_detect_language_auto'] = __('Enter a valid auto detect language value.', 'a11yfirsteditor_wordpress');
			}

			if (trim($new_options['advanced']['acf']) != 't' && trim($new_options['advanced']['acf'] != 'f')) {
				$massage['acf'] = __('Enter a valid ACF value.', 'a11yfirsteditor_wordpress');
			}
			...
			if (trim($new_options['advanced']['language_direction']) != 'default' && trim($new_options['advanced']['language_direction']) != 'ltr' && trim($new_options['advanced']['language_direction']) != 'rtl') {
				$massage['advanced_language_direction'] = __('Enter a valid language direction value.', 'a11yfirsteditor_wordpress');
			}
	...
	public function basic_settings_js() {
		wp_enqueue_script( 'a11yfirsteditor', $this->a11yfirsteditor_path .'ckeditor.js?t='.$this->timestamp, array(), $this->version);
		wp_enqueue_script( 'a11yfirsteditor_admin', $this->plugin_path . 'includes/basic.js');
	}
	...
	public function add_comment_js() {
		...
		// if W3 Total Cache is enabled, turn off minify for page with A11yFirstEditor in comments
		...
		wp_register_script('comment-reply', $this->plugin_path . 'includes/a11yfirsteditor.comment-reply.js', array('a11yfirsteditor', 'a11yfirsteditor.utils'), "20100901");
	...
	private function update_options($new_options, $error) {
		...
		if (!$error) {
			update_option("a11yfirsteditor_wordpress", $options);
			echo '<div class="updated"><p>' . __('Configuration updated!') . '</p></div>';
		}
		return $options;
	}
	...
	public function file_editor() {
		...
		if (isset($_POST['newcontent'])) {
			if (!wp_verify_nonce($_POST['csrf_a11yfirsteditor-for-wordpress'], 'a11yfirsteditor_create_nonce_file_editor') || empty($_POST['_wp_http_referer']) || ( isset($_SERVER['HTTP_REFERER']) && !strstr($_SERVER['HTTP_REFERER'], $_POST['_wp_http_referer']) )) {
				wp_die( "You do not have sufficient permissions to access this page." );
			}
	...
	private function generate_js_options($is_comment) {
		...
		$settings['templates_files'][] = $this->plugin_path . 'a11yfirsteditor.templates.js';
		$output = array(
		...
		switch ($options['css']['styles']) {
			case 'theme':
				$settings['stylesCombo_stylesSet'] = 'wordpress:' . get_template_directory_uri() . '/a11yfirsteditor.styles.js';
				break;
			case 'self':
				...
				break;
			default:
				$settings['stylesCombo_stylesSet'] = 'wordpress:' . $this->plugin_path . 'a11yfirsteditor.styles.js';
				break;
		}
		...
		$output['configuration']['customConfig'] = $this->plugin_path . 'a11yfirsteditor.config.js';
		if (!$is_comment) {
			$output['externalPlugins'] = apply_filters('a11yfirsteditor_external_plugins', array());
			$output['additionalButtons'] = apply_filters('a11yfirsteditor_buttons', array());
			...
			foreach ((array) $options['plugins'] as $name => $val) {
				if ($val == 't' && !isset($output['externalPlugins'][$name])) {
					...
					if (in_array($name, $available_plugins)) {
						$output['externalPlugins'][ $name ] = $this->plugin_path . 'a11yfirsteditor/plugins/' . $name . '/';
					}
				}
		...
		echo "<script type='text/javascript'>\n/* <![CDATA[ */\nwindow.A11FIRSTEDITOR_BASEPATH = \"" . $this->a11yfirsteditor_path . "\";\nvar a11yfirsteditorSettings = " . $this->jsEncode($output) . "\n/* ]]> */\n</script>";
	}
	...
	public function a11yfirsteditor_wpmore_plugin($plugins) {
		$plugins['wpmore'] = $this->plugin_path . 'plugins/wpmore/';
		return $plugins;
	}

	public function a11yfirsteditor_wpmore_button($buttons) {
		$buttons[] = array('WPMore');
		return $buttons;
	}

	public function a11yfirsteditor_wpgallery_plugin($plugins) {
		$plugins['wpgallery'] = $this->plugin_path . 'plugins/wpgallery/';
		return $plugins;
	}

	public function a11yfirsteditor_linkbrowser_plugin($plugins) {
		wp_nonce_field( 'internal-linking', '_ajax_linking_nonce', false );
		$plugins['linkbrowser'] = $this->plugin_path . 'plugins/linkbrowser/';
		return $plugins;
	}

	public function a11yfirsteditor_linkbrowser_loader() {
	...
	public function a11yfirsteditor_linkbrowser_search() {
	 ...
	 * @return mixed
	 */
	public function a11yfirsteditor_insert_post_data_filter($data, $postarr = null) {
	...
	/*public function a11yfirsteditor_externalvvq_plugin($plugins) {
	...
	public function a11yfirsteditor_vvqbuttons($buttons) {
		if (class_exists('VipersVideoQuicktags')) {
...
*/
	...
	/**
	 * List of installed a11yfirstEditor languages
	 *
	 * @return array
	 */
	public function a11yfirsteditor_load_lang_options() {
		$arr = array();
		$lang_file =  $this->a11yfirsteditor_path. 'lang/_languages.js';
		...
		//oops, we have no information about languages, let's use those available in a11yfirstEditor 2.4.3
		if (empty($arr)) {
			$arr = array(
				'af' => 'Afrikaans',
				'ar' => 'Arabic',
...
final class _WP_Editors {
	...
	public static function editor($content, $editor_id, $settings = array()) {
		...
		a11yfirsteditor_wordpress::getInstance()->editor_js();
	}
...
$a11yfirsteditor_wordpress = a11yfirsteditor_wordpress::getInstance();
